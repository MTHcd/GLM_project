...
$$X^2 = \sum_{j=1}^k \frac{(y_j - n_j (\frac{e^{\hat{\beta}_0 + \hat{\beta}_1x_j}}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}}))^2}{n_j (\frac{e^{\hat{\beta}_0 + \hat{\beta}_1x_j}}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}})(\frac{1}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}})} $$
...
$$ r_j = \frac{y_j - n_j (\frac{e^{\hat{\beta}_0 + \hat{\beta}_1x_j}}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}})}{\sqrt{n_j (\frac{e^{\hat{\beta}_0 + \hat{\beta}_1x_j}}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}})(\frac{1}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1x_j}})}} $$

## Probit Model

$$p=\Phi(\beta_1+\beta_2x), Phi(x) = \int_{-\infty}^x \frac{e^{-\frac{y^2}{2}}}{\sqrt{2\pi}}dy$$
...
$$ X^2 = \sum_{j=1}^k \frac{(y_j - n_j\hat{\pi}_j)^2}{n_j\hat{\pi}_j(1-\hat{\pi}_j)} \sim \chi^2_{(k-2)}$$

We obtain:

$$X^2 = \sum_{j=1}^k \frac{(y_j - n_j (\Phi(x_j\hat{\beta})))^2}{n_j(\Phi(x_j\hat{\beta}))(1- \Phi(x_j\hat{\beta}))} $$

#### Deviance Residuals

$$d_j = sign(y_j - n_j\hat{\pi}_j)\sqrt{2[y_jlog(\frac{y_j}{(n_j)\hat{\pi}_j})+(n_j-y_j)log(\frac{n_j-y_j}{(n_j)(1 - \hat{\pi}_j)})]}$$

#### Pearson Residuals

$$r_j = \frac{y_j - n_j\hat{\pi}_j}{\sqrt{\hat{Var(y_j)}}}$$


## Cloglog Model

$$log(-log(1-p))=\beta_1+\beta_2x \Leftrightarrow p(y=1|x) = 1 - e^{-e^{(\beta_1 + \beta_2 x)}}$$
...
$$X^2 = \sum_{j=1}^k \frac{(y_j - n_j (1 - e^{-e^{(\beta_1 + \beta_2 x_j)}}))^2}{n_j (1 - e^{-e^{(\beta_1 + \beta_2 x_j)}})(e^{-e^{(\beta_1 + \beta_2 x_j)}}} $$
...
EMV_algo <- function(beta, n) {
  ...
  if (log_lik(beta) < log_lik(new_beta)) {
    return(new_beta)
  }
  else if (log_lik(beta) >= log_lik(new_beta)){
      step <- 1
      repeat {
        new_beta <- beta + (I(beta))%*%(U(beta))
        if (log_lik(new_beta) > log_lik(beta) || step < 1e-6)
          break
        step <- step / 2
      }
      return(new_beta)
...
## 
AlgoEMV_algo_cloglog <- function(beta, n) {
