...
EMV_algo_probit <- function(beta, n) {
  ...
  if (log_lik_prob(beta) < log_lik_prob(new_beta)) {
    ...
    new_beta <- beta + (1/1000)*(I_probit(beta))%*%(U_probit(beta))
    if (log_lik_prob(beta) < log_lik_prob(new_beta)) {
      return(new_beta)
    } else {
      return(beta)
...
resu_EMV_Batch_probit <- function(beta_H1, beta_H2) {
  ...
  for (j in 2:N) {
    ...
    beta_H2[j] <- resu[2]
    param <- c(resu[1], resu[2])
    if (beta_H1[j-1] == beta_H1[j] & beta_H2[j-1] == beta_H2[j]) {
      return(c(beta_H1, beta_H2, z, j))
    }
...
# Exemple : coefficients GLM
glm_coef <- coef(glm(cbind(n1, n0) ~ x, family = binomial(link="probit")))
...
# Points donnés
p1 <- c(beta0 = -34, beta1 = 20)
p2 <- c(beta0 = -40, beta1 = 20)
...
# Mettre dans un data frame
df_line <- data.frame(beta0 = beta0_seq, beta1 = beta1_seq, log_lik_prob = loglik_seq)

# Tracer
ggplot(df_line, aes(x = beta0, y = beta1, fill = log_lik_prob)) +
  geom_tile() +
  scale_fill_viridis_c() +
  geom_point(aes(x = p1["beta0"], y = p1["beta1"]), color = "green", size = 3) +  # point 1
  geom_point(aes(x = p2["beta0"], y = p2["beta1"]), color = "red", size = 3) +    # point 2
